...
import { Bot, CheckCircle2, Search, Sparkles, Zap } from "lucide-react";
...
export default function HeroMockup() {
  ...
  return (
    <div className="relative w-full max-w-lg mx-auto lg:mx-0 lg:ml-auto">
      <div className="landing-mockup-shadow" aria-hidden="true" />

      <div className="relative landing-mockup-shell rounded-2xl p-1 animate-landing-float">
        <div className="rounded-[1.15rem] overflow-hidden bg-white border border-white/80">
          <div className="flex items-center gap-2 px-4 py-3 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
            <span className="w-2.5 h-2.5 rounded-full bg-red-300/90" />
            <span className="w-2.5 h-2.5 rounded-full bg-amber-300/90" />
            <span className="w-2.5 h-2.5 rounded-full bg-brand-secondary/90" />
            <span className="ml-3 text-[10px] font-mono text-slate-400 truncate flex-1">
              tramix.app/dashboard
            </span>
          </div>

          <div className="landing-mockup-body p-4 sm:p-5 space-y-4">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-bold text-brand-primary-dark">{t.home.mockGreeting}</p>
              <span className="flex items-center gap-1.5 text-[10px] font-bold text-brand-secondary-dark bg-brand-secondary/20 px-2 py-1 rounded-full">
            ...
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-brand-primary/45" />
              <div className="w-full pl-9 pr-4 py-3 rounded-xl bg-white border border-slate-200 text-xs text-slate-500 font-medium shadow-sm">
                {t.home.mockSearch}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div
                  key={item.code}
                  className="p-3 rounded-xl bg-white border border-slate-100 shadow-sm hover:border-brand-primary/25 transition-colors"
                >
                  ...
                  {item.online && (
                    <span className="inline-flex items-center gap-1 mt-2 text-[8px] font-bold text-brand-secondary-dark bg-brand-secondary/25 px-1.5 py-0.5 rounded-md">
      ...
      <div className="absolute -left-4 sm:-left-8 top-1/4 landing-float-card p-3 rounded-xl max-w-[140px] hidden sm:block animate-landing-float-delayed">
      ...
      <div className="absolute -right-2 sm:-right-6 bottom-8 landing-float-card p-3 rounded-xl hidden sm:block animate-landing-float">
        <div className="flex items-center gap-2">
